module FriendshipHelper
  def friendship_button(the_other_user)
    ...
    html = ''
    html << "<div>
              #{link_to( "Accept", accept_friendship_path(Friendship.where(user: the_other_user, friend: current_user).first), method: :post , class: "btn btn-success" )}
              #{link_to( "Reject", reject_friendship_path(Friendship.where(user: the_other_user, friend: current_user).first), method: :post , class: "btn btn-danger" )}
          </div>"
    html.html_safe
      # not friends
